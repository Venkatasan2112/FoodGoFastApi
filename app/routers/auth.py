...
from app.core.auth_utils import get_bearer_token,get_refresh_token,set_refresh_cookie,delete_refresh_cookie,raise_auth_error,raise_conflict_error
...
def login(login_data: LoginRequest,response: Response,db: Session = Depends(get_db)) -> LoginResponse:
    ...
    try:
        access_token, refresh_token = auth_service.login(db,login_data.email,login_data.password)
...
def refresh(request: Request,response: Response,db: Session = Depends(get_db)) -> LoginResponse:
    ...
    try:
        new_access_token, new_refresh_token = auth_service.refresh_access_token(db,access_token,refresh_token)
